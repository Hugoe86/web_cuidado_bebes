            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Modulo_ID + '" href="javascript:void(0)" data-modulos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Modulo_ID + '" href="javascript:void(0)" data-modulos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '   <div class="row">' +
            '       <div class="col-md-8">' +
            '            <label class="fuente_lbl_controles">Icon</label>' +
            '           <input type="text" name="txt_icono" id="txt_icono" class="icon-picker form-control-icono" disabled="true" readonly />' +
            '       </div>' +
            '       <div class="col-md-4">' +
            '            <label class="fuente_lbl_controles">(*) Order</label>' +
            '          <input type="text" id="txt_orden" name="txt_orden" class="form-control input-sm" disabled="disabled" placeholder="(*) Order" data-parsley-required="true" maxlength="100" required /> ' +
            '       </div>'
            '   </div>';
...
function _validation(opcion) {
    ...
    if (!$('#txt_orden').parsley().isValid()) {
        ...
        _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;Order is a required data.<br />';
    }
    if (!$('#txt_nombre').parsley().isValid()) {
        _add_class_error('#txt_nombre');
        _output.Estatus = false;
        _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;Name is a required data.<br />';
    } else {
