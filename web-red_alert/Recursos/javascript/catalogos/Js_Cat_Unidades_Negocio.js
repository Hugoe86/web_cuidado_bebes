...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_unidades_negocio').bootstrapTable('refresh', 'controllers/Unidades_Negocio_Controller.asmx/Consultar_Unidades_Negocio_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _habilitar_controles(opcion) {
    ...
    $('#cmb_plantas').attr({ disabled: !Estatus });
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_unidades_negocio').bootstrapTable('destroy');
        $('#tbl_unidades_negocio').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Unidad_Negocio_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Company', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Unidad_Negocio_ID + '" href="javascript:void(0)" data-unidades=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Unidad_Negocio_ID + '" href="javascript:void(0)" data-unidades=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_unidades() {
    var Unidades_Negocio = null;
    var isComplete = false;

    try {

        Unidades_Negocio = new Object();
        Unidades_Negocio.Nombre = $('#txt_nombre').val();
        Unidades_Negocio.Observaciones = $('#txt_observaciones').val();
        Unidades_Negocio.Estatus_ID = parseInt($('#cmb_estatus').val());
        Unidades_Negocio.Planta_ID = parseInt($('#cmb_plantas').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Unidades_Negocio) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Unidades_Negocio_Controller.asmx/Alta',
            data: $data,
...
function _modificar_unidades() {
    var Unidades_Negocio = null;
    var isComplete = false;

    try {
        Unidades_Negocio = new Object();
        Unidades_Negocio.Unidad_Negocio_ID = parseInt($('#txt_unidad_negocio_id').val());
        Unidades_Negocio.Nombre = $('#txt_nombre').val();
        Unidades_Negocio.Observaciones = $('#txt_observaciones').val();
        Unidades_Negocio.Estatus_ID = parseInt($('#cmb_estatus').val());
        Unidades_Negocio.Planta_ID = parseInt($('#cmb_plantas').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Unidades_Negocio_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_unidades(Unidad_Negocio_ID) {
    var Unidades_Negocio = null;

    try {
        Unidades_Negocio = new Object();
        Unidades_Negocio.Unidad_Negocio_ID = parseInt(Unidad_Negocio_ID);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Unidades_Negocio) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Unidades_Negocio_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_unidad_negocio_id"/>' +
            '       </div> ' +
            ...
            '        <select id="cmb_plantas" name="cmb_plantas" class="form-control input-sm" disabled="disabled" data-parsley-required="true" required ></select> ' +
            '   </div>' +
            '</div>' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_unidad_negocio_id').val() != null && $('#txt_unidad_negocio_id').val() != undefined && $('#txt_unidad_negocio_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_unidades()) {
                        _estado_inicial();
...
function btn_eliminar_click(Unidades_Negocio) {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_unidades(row.Unidad_Negocio_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Unidades_Negocio_Controller.asmx/Consultar_Unidades_Negocio_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_unidades_negocio').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_unidad_negocio_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_unidad_negocio_id').val(), 'estatus');
    ...
    if (!$('#cmb_plantas').parsley().isValid()) {
        _add_class_error('#cmb_plantas');
        _output.Estatus = false;
...
function _validate_fields(value, id, field) {
    ...
    try {
        Unidades_Negocio = new Object();
        if (id !== null)
            Unidades_Negocio.Unidad_Negocio_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Unidades_Negocio.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Unidades_Negocio_Controller.asmx/Consultar_Unidades_Negocio_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Unidades_Negocio_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
...
function _load_plantas() {
    var filtros = null;
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Unidades_Negocio_Controller.asmx/Consultar_Plantas',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
            success: function (datos) {
                if (datos !== null) {
                    var datos_combo = $.parseJSON(datos.d);
                    var select2 = $('#cmb_plantas');
                    $('option', select2).remove();
                    var options = '<option value="">--SELECT--</option>';
                    for (var Indice_plantas = 0; Indice_plantas < datos_combo.length; Indice_plantas++) {
                        options += '<option value="' + datos_combo[Indice_plantas].Planta_ID + '">' + datos_combo[Indice_plantas].Nombre.toUpperCase() + '</option>';
