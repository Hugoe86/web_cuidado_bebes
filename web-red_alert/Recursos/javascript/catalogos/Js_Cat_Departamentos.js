...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_departamentos').bootstrapTable('refresh', 'controllers/Departamentos_Controller.asmx/Consultar_Departamentos_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_departamentos').bootstrapTable('destroy');
        $('#tbl_departamentos').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Departamento_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Company', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Departamento_ID + '" href="javascript:void(0)" data-departamentos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Departamento_ID + '" href="javascript:void(0)" data-departamentos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_departamentos() {
    var Deparatamento = null;
    var isComplete = false;

    try {

        Deparatamento = new Object();
        Deparatamento.Nombre = $('#txt_nombre').val();
        Deparatamento.Observaciones = $('#txt_observaciones').val();
        Deparatamento.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Deparatamento) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Departamentos_Controller.asmx/Alta',
            data: $data,
...
function _modificar_departamentos() {
    var Deparatamento = null;
    var isComplete = false;

    try {
        Deparatamento = new Object();
        Deparatamento.Departamento_ID = parseInt($('#txt_departamento_id').val());
        Deparatamento.Nombre = $('#txt_nombre').val();
        Deparatamento.Observaciones = $('#txt_observaciones').val();
        Deparatamento.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Departamentos_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_departamentos(departamento_id) {
    var Deparatamento = null;

    try {
        Deparatamento = new Object();
        Deparatamento.Departamento_ID = parseInt(departamento_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Deparatamento) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Departamentos_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_departamento_id"/>' +
            '       </div> ' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_departamento_id').val() != null && $('#txt_departamento_id').val() != undefined && $('#txt_departamento_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_departamentos()) {
                        _estado_inicial();
...
function btn_editar_click(departamentos) {
    var row = $(departamentos).data('departamentos');

    $('#txt_departamento_id').val(row.Departamento_ID);
...
function btn_eliminar_click(departamentos) {
    var row = $(departamentos).data('departamentos');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_departamentos(row.Departamento_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Departamentos_Controller.asmx/Consultar_Departamentos_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_departamentos').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_departamento_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_departamento_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        Deparatamento = new Object();
        if (id !== null)
            Deparatamento.Departamento_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Deparatamento.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Departamentos_Controller.asmx/Consultar_Departamentos_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Departamentos_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
