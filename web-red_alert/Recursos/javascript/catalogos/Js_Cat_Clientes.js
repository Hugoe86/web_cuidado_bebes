...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_clientes').bootstrapTable('refresh', 'controllers/Clientes_Controller.asmx/Consultar_Clientes_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_clientes').bootstrapTable('destroy');
        $('#tbl_clientes').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Cliente_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Company', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Cliente_ID + '" href="javascript:void(0)" data-clientes=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Cliente_ID + '" href="javascript:void(0)" data-clientes=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_clientes() {
    ...
    try {
        ...
        Clientes = new Object();
        Clientes.Nombre = $('#txt_nombre').val();
        Clientes.Apellidos = $('#txt_apellidos').val();
        Clientes.Observaciones = $('#txt_observaciones').val();
        Clientes.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Clientes) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Clientes_Controller.asmx/Alta',
            data: $data,
...
function _modificar_clientes() {
    var Clientes = null;
    var isComplete = false;

    try {
        Clientes = new Object();
        Clientes.Cliente_ID = parseInt($('#txt_cliente_id').val());
        Clientes.Nombre = $('#txt_nombre').val();
        Clientes.Apellidos = $('#txt_apellidos').val();
        Clientes.Observaciones = $('#txt_observaciones').val();
        Clientes.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Clientes_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_clientes(Cliente_ID) {
    var Clientes = null;

    try {
        Clientes = new Object();
        Clientes.Cliente_ID = parseInt(Cliente_ID);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Clientes) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Clientes_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '       <input type="text" id="txt_apellidos" name="txt_apellidos" class="form-control input-sm" disabled="disabled" placeholder="Last name" maxlength="100" />' +
            '   </div>' +
            '</div>' +
...
function btn_editar_click(clientes) {
    var row = $(clientes).data('clientes');

    $('#txt_cliente_id').val(row.Cliente_ID);
...
function btn_eliminar_click(clientes) {
    var row = $(clientes).data('clientes');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_clientes(row.Cliente_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Clientes_Controller.asmx/Consultar_Clientes_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_clientes').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_cliente_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_cliente_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        Clientes = new Object();
        if (id !== null)
            Clientes.Cliente_ID = parseInt(id);
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Clientes_Controller.asmx/Consultar_Clientes_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
