...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_grupos').bootstrapTable('refresh', 'controllers/Grupos_Criterios_Controller.asmx/Consultar_Grupos_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_grupos').bootstrapTable('destroy');
        $('#tbl_grupos').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Grupo_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Observaciones', title: 'Observations', width: 200, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Grupo_ID + '" href="javascript:void(0)" data-grupos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Grupo_ID + '" href="javascript:void(0)" data-grupos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_grupos() {
    ...
    try {
        ...
        Grupos = new Object();
        Grupos.Nombre = $('#txt_nombre').val();
        Grupos.Observaciones = $('#txt_observaciones').val();
        Grupos.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Grupos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Grupos_Criterios_Controller.asmx/Alta',
            data: $data,
...
function _modificar_grupos() {
    var Grupos = null;
    var isComplete = false;

    try {
        Grupos = new Object();
        Grupos.Grupo_ID = parseInt($('#txt_grupo_id').val());
        Grupos.Nombre = $('#txt_nombre').val();
        Grupos.Observaciones = $('#txt_observaciones').val();
        Grupos.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Grupos_Criterios_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_grupos(grupo_id) {
    var Grupos = null;

    try {
        Grupos = new Object();
        Grupos.Grupo_ID = parseInt(grupo_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Grupos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Grupos_Criterios_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_grupo_id"/>' +
            '       </div> ' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_grupo_id').val() != null && $('#txt_grupo_id').val() != undefined && $('#txt_grupo_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_grupos()) {
                        _estado_inicial();
...
function btn_editar_click(grupos) {
    var row = $(grupos).data('grupos');

    $('#txt_grupo_id').val(row.Grupo_ID);
...
function btn_eliminar_click(grupos) {
    var row = $(grupos).data('grupos');
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Grupos_Criterios_Controller.asmx/Consultar_Grupos_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_grupos').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_grupo_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_grupo_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Grupos_Criterios_Controller.asmx/Consultar_Grupos_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Grupos_Criterios_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
