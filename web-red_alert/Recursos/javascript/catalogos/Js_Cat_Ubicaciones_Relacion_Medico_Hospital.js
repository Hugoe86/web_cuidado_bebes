...
function _consultar_relacion() {
    ...
    try {
        ...
        filtros.Medico_Id = 0;
        filtros.Hospital_Id = parseInt($('#txt_ubicacion_relacion_id').val());
...
function _limpiar_todos_controles_modal_relacion() {
    ...
    try {
       ...
       =============================================*/
        ...
        $('#cmb_medicos').empty().trigger("change");
...
/* =============================================
--NOMBRE_FUNCIÓN:       _load_cmb_medicos
...
--CAUSA_MODIFICACIÓN:
=============================================*/
function _load_cmb_medicos(cmb) {
    try {
        ...
        $('#' + cmb).select2({
            ...
            allowClear: true,
            ajax: {
                url: 'controllers/MedicosController.asmx/Consultar_Medicos_Combo',
                cache: true,
                dataType: 'json',
                type: "POST",
                delay: 250,
        ...
        //  se muestra el mensaje del error que se presento
        _mostrar_mensaje('Informe técnico' + '[_load_cmb_medicos]', e);
    }
}
...
function _validar_relacion() {
    ...
    try {
        ...
        if ($('#cmb_medico :selected').val() === undefined || $('#cmb_medico :selected').val() === '') {
            //  agregamos el estilo al control que no cumple
            _output.Estatus = false;
            _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;El médico.<br />';
...
function alta_relacion() {
    ...
    try {
        ...
        //  se carga la información 
        obj.Medico_Id = parseInt($('#cmb_medico :selected').val());
        obj.Hospital_Id = parseInt($('#txt_ubicacion_relacion_id').val());
    ...
    try {
        ...
        $('#tbl_relacion').bootstrapTable({
            ...
            columns: [
                ...
                { field: 'Medico', title: 'Medico', align: 'left', valign: 'top', width: 150, visible: true, sortable: true },
                { field: 'Hospital', title: 'Hospital', align: 'left', valign: 'top', width: 150, visible: false, sortable: true },
