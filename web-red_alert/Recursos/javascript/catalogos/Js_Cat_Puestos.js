            ...
            columns: [
                ...
                { field: 'Descripcion', title: 'Description', width: 200, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Puesto_ID + '" href="javascript:void(0)" data-puestos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Puesto_ID + '" href="javascript:void(0)" data-puestos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <textarea  id="txt_descripcion" name="txt_descripcion" class="form-control input-sm" disabled="disabled" placeholder="Description"  style="min-height: 50px !important;" data-parsley-required="false" maxlength="250"> </textarea>' +
            '    </div>' +
            '</div>';
...
function _validation(opcion) {
    ...
    if (!$('#cmb_areas').parsley().isValid()) {
        ...
        _output.Estatus = false;
        _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;The area is a required data.<br />';
    }
...
function _validate_fields_areas(value, id, field, area_id) {
    ...
    try {
        ...
        Resultado = new Object();
        Resultado.Estatus = 'error';
        Resultado.Mensaje = 'It was not possible to validate thel ' + field + ' in the database.';
        _mostrar_mensaje('Technical report', e);
    }
    return Resultado;
}
