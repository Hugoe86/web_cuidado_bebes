...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_sitios_clientes').bootstrapTable('refresh', 'controllers/Sitios_Clientes_Controller.asmx/Consultar_Sitios_Clientes_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Informe Técnico', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_sitios_clientes').bootstrapTable('destroy');
        $('#tbl_sitios_clientes').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Sitio_Cliente_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Empresa', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Sitio_Cliente_ID + '" href="javascript:void(0)" data-sitios_clientes=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Sitio_Cliente_ID + '" href="javascript:void(0)" data-sitios_clientes=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_sitios() {
    var Sitios_Clientes = null;
    var isComplete = false;

    try {

        Sitios_Clientes = new Object();
        Sitios_Clientes.Nombre = $('#txt_nombre').val();
        Sitios_Clientes.Observaciones = $('#txt_observaciones').val();
        Sitios_Clientes.Estatus_ID = parseInt($('#cmb_estatus').val());
        Sitios_Clientes.Cliente_ID = parseInt($('#cmb_clientes').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Sitios_Clientes) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Sitios_Clientes_Controller.asmx/Alta',
            data: $data,
...
function _modificar_sitios() {
    var Sitios_Clientes = null;
    var isComplete = false;

    try {
        Sitios_Clientes = new Object();
        Sitios_Clientes.Sitio_Cliente_ID = parseInt($('#txt_sitio_cliente_id').val());
        Sitios_Clientes.Nombre = $('#txt_nombre').val();
        Sitios_Clientes.Observaciones = $('#txt_observaciones').val();
        Sitios_Clientes.Estatus_ID = parseInt($('#cmb_estatus').val());
        Sitios_Clientes.Cliente_ID = parseInt($('#cmb_clientes').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Sitios_Clientes_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_sitios(sitio_cliente_id) {
    var Sitios_Clientes = null;

    try {
        Sitios_Clientes = new Object();
        Sitios_Clientes.Sitio_Cliente_ID = parseInt(sitio_cliente_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Sitios_Clientes) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Sitios_Clientes_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_sitio_cliente_id"/>' +
            '       </div> ' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_sitio_cliente_id').val() != null && $('#txt_sitio_cliente_id').val() != undefined && $('#txt_sitio_cliente_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_sitios()) {
                        _estado_inicial();
...
function btn_editar_click(sitios_clientes) {
    var row = $(sitios_clientes).data('sitios_clientes');

    $('#txt_sitio_cliente_id').val(row.Sitio_Cliente_ID);
...
function btn_eliminar_click(sitios_clientes) {
    var row = $(sitios_clientes).data('sitios_clientes');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_sitios(row.Sitio_Cliente_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Sitios_Clientes_Controller.asmx/Consultar_Sitios_Clientes_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_sitios_clientes').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_sitio_cliente_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_sitio_cliente_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        Sitios_Clientes = new Object();
        if (id !== null)
            Sitios_Clientes.Sitio_Cliente_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Sitios_Clientes.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Sitios_Clientes_Controller.asmx/Consultar_Sitios_Clientes_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Sitios_Clientes_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
...
function _load_clientes() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Sitios_Clientes_Controller.asmx/Consultar_Clientes',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
