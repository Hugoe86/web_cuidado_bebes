...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_procesos').bootstrapTable('refresh', 'controllers/Procesos_Controller.asmx/Consultar_Procesos_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        ...
        $('#tbl_procesos').bootstrapTable({
            ...
            clickToSelect: true,
            columns: [
                { field: 'Proceso_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Company', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Proceso_ID + '" href="javascript:void(0)" data-procesos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Proceso_ID + '" href="javascript:void(0)" data-procesos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_procesos() {
    ...
    try {
        ...
        Procesos = new Object();
        Procesos.Nombre = $('#txt_nombre').val();
        Procesos.Observaciones = $('#txt_observaciones').val();
        Procesos.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Procesos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Procesos_Controller.asmx/Alta',
            data: $data,
...
function _modificar_procesos() {
    var Procesos = null;
    var isComplete = false;

    try {
        Procesos = new Object();
        Procesos.Proceso_ID = parseInt($('#txt_proceso_id').val());
        Procesos.Nombre = $('#txt_nombre').val();
        Procesos.Observaciones = $('#txt_observaciones').val();
        Procesos.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Procesos_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_procesos(Proceso_ID) {
    var Procesos = null;

    try {
        Procesos = new Object();
        Procesos.Proceso_ID = parseInt(Proceso_ID);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Procesos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Procesos_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_proceso_id"/>' +
            '       </div> ' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_proceso_id').val() != null && $('#txt_proceso_id').val() != undefined && $('#txt_proceso_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_procesos()) {
                        _estado_inicial();
...
function btn_editar_click(Procesos) {
    var row = $(Procesos).data('procesos');

    $('#txt_proceso_id').val(row.Proceso_ID);
...
function btn_eliminar_click(Procesos) {
    var row = $(Procesos).data('procesos');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_procesos(row.Proceso_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Procesos_Controller.asmx/Consultar_Procesos_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_proceso_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_proceso_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        Procesos = new Object();
        if (id !== null)
            Procesos.Proceso_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Procesos.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Procesos_Controller.asmx/Consultar_Procesos_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Procesos_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
