...
function _load_vistas() {
    _launchComponent('vistas/Vacunas/Principal.html', 'Principal');
    _launchComponent('vistas/Vacunas/Modal.html', 'Modal');
}
...
function crear_tabla() {
    ...
    try {
        $('#tbl_vacunas').bootstrapTable('destroy');
        $('#tbl_vacunas').bootstrapTable({
            cache: false,
            striped: true,
            pagination: true,
            data: [],
            ...
            columns: [
                { field: 'Vacuna_Id', title: 'Vacuna_Id', align: 'center', valign: 'top', visible: false },
                ...
                //{ field: 'Meses_Aplicacion', title: 'No. Meses', align: 'left', valign: 'top', visible: true, sortable: true },
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        opciones += '<div style="display:block"><a class="remove ml10 text-purple" id="' + row.Vacuna_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i>&nbsp;<span style="font-size:11px !important;">Editar</span></a></div>';
                        opciones += '</div>';
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        var opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-red" id="' + row.Vacuna_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Editar"><i class="glyphicon glyphicon-trash"></i>&nbsp;<span style="font-size:11px !important;">Eliminar</span></a></div>';
                        opciones += '</div>';
...
function _ConsultarFiltros() {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/VacunasController.asmx/Consultar_VacunasFiltro',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
            ...
            success: function (datos) {
                if (datos !== null) {
                    ...
                    $('#tbl_vacunas').bootstrapTable('load', datos);
                }
            }
        });
    } catch (e) {
...
function alta() {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/VacunasController.asmx/Alta',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function actualizar() {
    ...
    try {
        ...
        obj.Vacuna_Id = parseInt($('#txt_vacuna_id').val());
        obj.Estatus_Id = parseInt($('#cmb_estatus').val());
        obj.Nombre = $('#txt_nombre').val();
        //obj.Meses_Aplicacion = parseInt($('#txt_meses').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/VacunasController.asmx/Actualizar',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function btn_editar_click(tab) {
    ...
    $('#txt_vacuna_id').val(row.Vacuna_Id);
    //$('#txt_meses').val(row.Meses_Aplicacion);
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                filtros.Vacuna_Id = row.Vacuna_Id;
                var $data = JSON.stringify({ 'jsonObject': JSON.stringify(filtros) });

                $.ajax({
                    type: 'POST',
                    url: 'controllers/VacunasController.asmx/Eliminar',
                    data: $data,
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    async: false,
