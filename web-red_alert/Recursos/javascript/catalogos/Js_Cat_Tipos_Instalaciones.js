...
function _load_vistas() {
    _launchComponent('vistas/Tipos/Principal.html', 'Principal');
    _launchComponent('vistas/Tipos/Modal.html', 'Modal');
}
...
function crear_tabla() {
    ...
    try {
        $('#tbl_tipos').bootstrapTable('destroy');
        $('#tbl_tipos').bootstrapTable({
            cache: false,
            striped: true,
            pagination: true,
            data: [],
            ...
            columns: [
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        opciones += '<div style="display:block"><a class="remove ml10 text-purple" id="' + row.Tipo_Instalacion_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i>&nbsp;<span style="font-size:11px !important;">Editar</span></a></div>';
                        opciones += '</div>';
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        var opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-red" id="' + row.Tipo_Instalacion_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Editar"><i class="glyphicon glyphicon-trash"></i>&nbsp;<span style="font-size:11px !important;">Eliminar</span></a></div>';
                        opciones += '</div>';
...
function _ConsultarFiltros() {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/TiposInstalacionesController.asmx/Consultar_TiposFiltro',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
            ...
            success: function (datos) {
                if (datos !== null) {
                    ...
                    $('#tbl_tipos').bootstrapTable('load', datos);
                }
            }
        });
    } catch (e) {
...
function alta() {
    ...
    try {
        ...
        obj.Meses_Aplicacion = parseInt($('#txt_meses').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(obj) });

        $.ajax({
            type: 'POST',
            url: 'controllers/TiposInstalacionesController.asmx/Alta',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function actualizar() {
    ...
    try {
        ...
        obj.Tipo_Instalacion_Id = parseInt($('#txt_tipo_id').val());
        obj.Estatus_Id = parseInt($('#cmb_estatus').val());
        obj.Nombre = $('#txt_nombre').val();
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/TiposInstalacionesController.asmx/Actualizar',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function btn_editar_click(tab) {
    ...
    $('#txt_tipo_id').val(row.Tipo_Instalacion_Id);
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                filtros.Tipo_Instalacion_Id = row.Tipo_Instalacion_Id;
                var $data = JSON.stringify({ 'jsonObject': JSON.stringify(filtros) });

                $.ajax({
                    type: 'POST',
                    url: 'controllers/TiposInstalacionesController.asmx/Eliminar',
                    data: $data,
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    async: false,
...
function _launch_modal(title_window) {
    ...
    jQuery('#modal_tipos').modal('show', { backdrop: 'static', keyboard: false });
}
