...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_vehiculos').bootstrapTable('refresh', 'controllers/Vehiculos_Controller.asmx/Consultar_Vehiculos_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Informe Técnico', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        ...
        $('#tbl_vehiculos').bootstrapTable({
            ...
            clickToSelect: true,
            columns: [
                { field: 'Vehiculo_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                { field: 'Nombre', title: 'Nombre', width: 150, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                { field: 'Sitio_Cliente', title: 'Sitio', width: 100, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                ...
                { field: 'Empresa_ID', title: 'Empresa', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Vehiculo_ID + '" href="javascript:void(0)" data-vehiculos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Vehiculo_ID + '" href="javascript:void(0)" data-vehiculos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_vehiculos() {
    var Vehiculos = null;
    var isComplete = false;

    try {

        Vehiculos = new Object();
        Vehiculos.Nombre = $('#txt_nombre').val();
        Vehiculos.Observaciones = $('#txt_observaciones').val();
        Vehiculos.Estatus_ID = parseInt($('#cmb_estatus').val());
        Vehiculos.Sitio_Cliente_ID = parseInt($('#cmb_sitios_clientes').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Vehiculos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Vehiculos_Controller.asmx/Alta',
            data: $data,
...
function _modificar_vehiculos() {
    var Vehiculos = null;
    var isComplete = false;

    try {
        Vehiculos = new Object();
        Vehiculos.Vehiculo_ID = parseInt($('#txt_vehiculo_id').val());
        Vehiculos.Nombre = $('#txt_nombre').val();
        Vehiculos.Observaciones = $('#txt_observaciones').val();
        Vehiculos.Estatus_ID = parseInt($('#cmb_estatus').val());
        Vehiculos.Sitio_Cliente_ID = parseInt($('#cmb_sitios_clientes').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Vehiculos_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_vehiculos(vehiculo_id) {
    var Vehiculos = null;

    try {
        Vehiculos = new Object();
        Vehiculos.Vehiculo_ID = parseInt(vehiculo_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Vehiculos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Vehiculos_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_vehiculo_id"/>' +
            '       </div> ' +
            ...
            '<div class="row">' +
            '   <div class="col-md-6">' +
            '            <label class="fuente_lbl_controles">(*) Sitio Cliente</label>' +
            '        <select id="cmb_sitios_clientes" name="cmb_sitios_clientes" class="form-control input-sm" disabled="disabled" data-parsley-required="true" required /> ' +
            '   </div>' +
            '</div>' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_vehiculo_id').val() != null && $('#txt_vehiculo_id').val() != undefined && $('#txt_vehiculo_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_vehiculos()) {
                        _estado_inicial();
...
function btn_editar_click(vehiculos) {
    var row = $(vehiculos).data('vehiculos');
...
function btn_eliminar_click(vehiculos) {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_vehiculos(row.Vehiculo_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Vehiculos_Controller.asmx/Consultar_Vehiculos_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_vehiculos').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_vehiculo_id').val(), 'nombre');
    ...
    if (!$('#cmb_sitios_clientes').parsley().isValid()) {
        _add_class_error('#cmb_sitios_clientes');
        _output.Estatus = false;
        _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;El sitio cliente es un dato requerido.<br />';
    }
...
function _validate_fields(value, id, field) {
    ...
    try {
        Vehiculos = new Object();
        if (id !== null)
            Vehiculos.Vehiculo_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Vehiculos.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Vehiculos_Controller.asmx/Consultar_Vehiculos_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Vehiculos_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
...
function _load_sitios_clientes() {
    var filtros = null;
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Vehiculos_Controller.asmx/Consultar_Sitios_Clientes',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
            success: function (datos) {
                if (datos !== null) {
                    var datos_combo = $.parseJSON(datos.d);
                    var select2 = $('#cmb_sitios_clientes');
                    $('option', select2).remove();
                    var options = '<option value="">--SELECCIONE--</option>';
                    for (var Indice_sitios = 0; Indice_sitios < datos_combo.length; Indice_sitios++) {
                        options += '<option value="' + datos_combo[Indice_sitios].Sitio_Cliente_ID + '">' + datos_combo[Indice_sitios].Nombre.toUpperCase() + '</option>';
