...
function _load_vistas() {
    _launchComponent('vistas/Registro_Civil/Principal.html', 'Principal');
    _launchComponent('vistas/Registro_Civil/Operacion.html', 'Operacion');
    _launchComponent('vistas/Registro_Civil/Relacion.html', 'Relacion');
}
...
function crear_tabla_horarios() {
    ...
    try {
        $('#tbl_horarios_registro_civil').bootstrapTable('destroy');
        $('#tbl_horarios_registro_civil').bootstrapTable({
            cache: false,
            striped: true,
            pagination: false,
            data: [],
            ...
            columns: [
                { field: 'Horario_Registro_Civil_Id', title: 'Horario_Registro_Civil_Id', align: 'center', valign: 'top', visible: false },
                { field: 'Registro_Civil_Id', title: 'Registro_Civil_Id', align: 'center', valign: 'top', visible: false },
...
function crear_tabla() {
    ...
    try {
        $('#tbl_registro_civil').bootstrapTable('destroy');
        $('#tbl_registro_civil').bootstrapTable({
            cache: false,
            striped: true,
            pagination: true,
            data: [],
            ...
            columns: [
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        var opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-purple" id="' + row.Registro_Civil_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i>&nbsp;<span style="font-size:11px !important;"></span></a></div>';
                        opciones += '</div>';
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        var opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-red" id=edti_"' + row.Registro_Civil_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Editar"><i class="glyphicon glyphicon-trash"></i>&nbsp;<span style="font-size:11px !important;"></span></a></div>';
                        opciones += '</div>';
...
function _ConsultarFiltros() {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Registro_Civil_Controller.asmx/Consultar_Registros_Filtro',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
            ...
            success: function (datos) {
                if (datos !== null) {
                    ...
                    $('#tbl_registro_civil').bootstrapTable('load', datos);
                }
            }
        });
    } catch (e) {
...
function _Consultar_Horarios(id) {
    var filtros = null;
    try {
        filtros = new Object();

        filtros.Registro_Civil_Id = parseInt(id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(filtros) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Registro_Civil_Controller.asmx/Consultar_Horarios_Registro_Civil',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
            ...
            success: function (datos) {
                if (datos !== null) {
                    datos = JSON.parse(datos.d);
                    $('#tbl_horarios_registro_civil').bootstrapTable('load', datos);
                }
            }
        });
    } catch (e) {
...
function _limpiar_todos_controles_procesos() {
    ...
    try {
        ...
        $('#cmb_estatus').val('');
        $("#tbl_horarios_registro_civil").bootstrapTable('load', []);
        _limpiar_iframe();
...
function _validarDatos_Nuevo() {
    ...
    try {
        ...
        //    _output.Estatus = false;
        //    _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;La hora de inicio.<br />';
        //}

        //if ($('#txt_hora_termino').val() == '' || $('#txt_hora_termino').val() == undefined) {
        //    _output.Estatus = false;
        //    _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;La hora de termino.<br />';
        //}
        ...
        /*var tbl = $('#tbl_horarios_registro_civil').bootstrapTable('getData');
        if (tbl == 0) {
            _output.Estatus = false;
...
function alta() {
    ...
    try {
        ...
        obj.Tramite = $('#txt_tramites').val();
        //obj.Emergencias = $('#ck_urgencias').prop('checked');

        obj.Lista_Horarios = JSON.stringify($('#tbl_horarios_registro_civil').bootstrapTable('getData'));

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(obj) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Registro_Civil_Controller.asmx/Alta',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function actualizar() {
    ...
    try {
        ...
        obj.Registro_Civil_Id = $('#txt_registro_civil_id').val();
        obj.Estatus_Id = $('#cmb_estatus').val();
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Registro_Civil_Controller.asmx/Actualizar',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function btn_editar_click(tab) {
    ...
    $('#txt_registro_civil_id').val(row.Registro_Civil_Id);
    ...
    $('#txt_tramites').val(row.Tramite);

    
    _Consultar_Horarios(row.Registro_Civil_Id);
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                filtros.Registro_Civil_Id = row.Registro_Civil_Id;
                var $data = JSON.stringify({ 'jsonObject': JSON.stringify(filtros) });

                $.ajax({
                    type: 'POST',
                    url: 'controllers/Registro_Civil_Controller.asmx/Eliminar',
                    data: $data,
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    async: false,
...
function Limpiar_Puntos() {
    if (nuevo != 2) {
        //$('#txt_latitud_origen').val('')
        //$('#txt_longitud_origen').val('')
        //$('#txt_latitud_destino').val('')
        //$('#txt_longitud_destino').val('')
...
function btn_relacionar_click(tab) {
    ...
    $('#txt_ubicacion_relacion_id').val(row.Registro_Civil_Id);
...
function _agregar_horarios() {
    ...
    try {
        $('#tbl_horarios_registro_civil').bootstrapTable('insertRow', {
            index: 0,
            row: {
                Horario_Registro_Civil_Id: 0,
                Registro_Civil_Id: $("#txt_registro_civil_id").val(),
                Dia: $("#cmb_dia").val(),
...
function btn_agregar_horarios() {
    try {
        if (!_validar_duplicar_horarios($('#cmb_dia').val())) {
            ...
            _mostrar_mensaje('Información', 'El día seleccionado ya fue agregado a la lista.');
        }
    } catch (e) {
        _mostrar_mensaje('Informe Técnico', e);
    }
...
function _validar_duplicar_horarios(dia) {
    ...
    try {
        _agregados = $("#tbl_horarios_registro_civil").bootstrapTable('getData');
...
function _btn_quitar(orden) {
    var indexrow = null;
    var row = $(orden).data('orden');
    try {
            if (row.Estatus == 'Nuevo') {
                $('#tbl_horarios_registro_civil').bootstrapTable('remove', {
                    field: 'Dia',
                ...
                eliminar.Horario_Registro_Civil_Id = parseInt(row.Horario_Registro_Civil_Id);
