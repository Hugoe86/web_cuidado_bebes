...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_productos').bootstrapTable('refresh', 'controllers/Productos_Controller.asmx/Consultar_Productos_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_productos').bootstrapTable('destroy');
        $('#tbl_productos').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Producto_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Company', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Producto_ID + '" href="javascript:void(0)" data-productos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Producto_ID + '" href="javascript:void(0)" data-productos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_productos() {
    ...
    try {
        ...
        Productos.Nombre = $('#txt_nombre').val();
        Productos.Observaciones = $('#txt_observaciones').val();
        Productos.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Productos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Productos_Controller.asmx/Alta',
            data: $data,
...
function _modificar_productos() {
    var Productos = null;
    var isComplete = false;

    try {
        Productos = new Object();
        Productos.Producto_ID = parseInt($('#txt_producto_id').val());
        Productos.Nombre = $('#txt_nombre').val();
        Productos.Observaciones = $('#txt_observaciones').val();
        Productos.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Productos_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_productos(Producto_ID) {
    var Productos = null;

    try {
        Productos = new Object();
        Productos.Producto_ID = parseInt(Producto_ID);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Productos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Productos_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_producto_id').val() != null && $('#txt_producto_id').val() != undefined && $('#txt_producto_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_productos()) {
                        _estado_inicial();
...
function btn_editar_click(Productos) {
    var row = $(Productos).data('productos');

    $('#txt_producto_id').val(row.Producto_ID);
...
function btn_eliminar_click(Productos) {
    var row = $(Productos).data('productos');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_productos(row.Producto_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Productos_Controller.asmx/Consultar_Productos_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_productos').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_producto_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_producto_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        Productos = new Object();
        if (id !== null)
            Productos.Producto_ID = parseInt(id);
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Productos_Controller.asmx/Consultar_Productos_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Productos_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
