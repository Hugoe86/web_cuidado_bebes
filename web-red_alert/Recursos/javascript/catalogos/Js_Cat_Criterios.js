/// <reference path="C:\Proyecto_Red_Alert\red-alert\web-red_alert\Paginas/Catalogos/controllers/Turnos_Controller.asmx" />
var closeModal = true;
var estatusActivo = '';
...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_criterios').bootstrapTable('refresh', 'controllers/Criterios_Controller.asmx/Consultar_Criterios_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_criterios').bootstrapTable('destroy');
        $('#tbl_criterios').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Criterio_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Descripcion', title: 'Criterion Type', width: 200, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                {
                    field: 'Criterio_ID',
                    title: '',
                    align: 'center',
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Criterio_ID + '" href="javascript:void(0)" data-criterios=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Criterio_ID + '" href="javascript:void(0)" data-criterios=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_criterios() {
    ...
    try {
        ...
        Criterios.Descripcion = $('#cmb_tipo_criterio').val();

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Criterios) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Criterios_Controller.asmx/Alta',
            data: $data,
...
function _modificar_criterios() {
    ...
    try {
        Criterios = new Object();
        Criterios.Criterio_ID = parseInt($('#txt_criterio_id').val());
        Criterios.Nombre = $('#txt_nombre').val();
        Criterios.Descripcion = $('#cmb_tipo_criterio').val();
        Criterios.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Criterios_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_criterios(criterio_id) {
    ...
    try {
        Criterios = new Object();
        Criterios.Criterio_ID = parseInt(criterio_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Criterios) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Criterios_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_criterio_id"/>' +
            '       </div> ' +
            ...
            '       <select id="cmb_tipo_criterio" name="cmb_tipo_criterio" class="form-control input-sm" disabled="disabled" data-parsley-required="true" required>' +
            '           <option value="">--SELECT--</option><option value="Calidad">Calidad</option><option value="Lanzador">Lanzador</option>' +
            '       </select>' +
            '   </div>' +
            '</div>';
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_criterio_id').val() != null && $('#txt_criterio_id').val() != undefined && $('#txt_criterio_id').val() != '') {
...
function btn_editar_click(criterios) {
    var row = $(criterios).data('criterios');

    $('#txt_criterio_id').val(row.Criterio_ID);
    $('#txt_nombre').val(row.Nombre);
    $('#cmb_tipo_criterio').val(row.Descripcion);
...
function btn_eliminar_click(criterios) {
    var row = $(criterios).data('criterios');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                _eliminar_criterios(row.Criterio_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Criterios_Controller.asmx/Consultar_Criterios_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_criterios').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_criterio_id').val(), 'nombre');
    ...
    if (!$('#cmb_tipo_criterio').parsley().isValid()) {
        _add_class_error('#cmb_tipo_criterio');
        _output.Estatus = false;
        _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;The criterion type is a required data.<br />';
    }
...
function _validate_fields(value, id, field) {
    ...
    try {
        Criterios = new Object();
        if (id !== null)
            Criterios.Criterio_ID = parseInt(id);
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Criterios_Controller.asmx/Consultar_Criterios_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
//function _load_tipo_criterio() {
//    try{
//        var select3 = $('#cmb_tipo_criterio');
//        $('option', select3).remove();
//        var options = '<option value="">--SELECCIONE--</option>';
//        options += '<option value="Calidad">Calidad</option>';
//        options += '<option value="Lanzador">Lanzador</option>';
//        $('#cmb_tipo_criterio').append(options);
//    }catch(e){
