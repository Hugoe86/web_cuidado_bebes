...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_turnos').bootstrapTable('refresh', 'controllers/Turnos_Controller.asmx/Consultar_Turnos_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_turnos').bootstrapTable('destroy');
        $('#tbl_turnos').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Turno_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Empresa_ID', title: 'Company', width: 100, align: 'left', valign: 'bottom', sortable: true, visible: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Turno_ID + '" href="javascript:void(0)" data-turnos=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Turno_ID + '" href="javascript:void(0)" data-turnos=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_turnos() {
    ...
    try {
        ...
        Turnos = new Object();
        Turnos.Nombre = $('#txt_nombre').val();
        Turnos.Observaciones = $('#txt_observaciones').val();
        Turnos.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Turnos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Turnos_Controller.asmx/Alta',
            data: $data,
...
function _modificar_turnos() {
    var Turnos = null;
    var isComplete = false;

    try {
        Turnos = new Object();
        Turnos.Turno_ID = parseInt($('#txt_turno_id').val());
        Turnos.Nombre = $('#txt_nombre').val();
        Turnos.Observaciones = $('#txt_observaciones').val();
        Turnos.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Turnos_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_turnos(turno_id) {
    var Turnos = null;

    try {
        Turnos = new Object();
        Turnos.Turno_ID = parseInt(turno_id);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Turnos) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Turnos_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_turno_id"/>' +
            '       </div> ' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_turno_id').val() != null && $('#txt_turno_id').val() != undefined && $('#txt_turno_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_turnos()) {
                        _estado_inicial();
...
function btn_editar_click(turnos) {
    var row = $(turnos).data('turnos');

    $('#txt_turno_id').val(row.Turno_ID);
...
function btn_eliminar_click(turnos) {
    var row = $(turnos).data('turnos');
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Turnos_Controller.asmx/Consultar_Turnos_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_turnos').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_turno_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_turno_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Turnos_Controller.asmx/Consultar_Turnos_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Turnos_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
