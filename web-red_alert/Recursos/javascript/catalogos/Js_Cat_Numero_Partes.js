...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_numero_partes').bootstrapTable('refresh', 'controllers/Numero_Partes_Controller.asmx/Consultar_Numero_Parte_Por_Filtros');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Technical report', e);
    }
...
function _eventos_textbox() {
    $('#txt_nombre').on('blur', function () {
        $(this).val($(this).val().match(/^[^'#&\\]*$/) ? $(this).val() : $(this).val().replace(/\W+/g, ''));
    });
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_numero_partes').bootstrapTable('destroy');
        $('#tbl_numero_partes').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            clickToSelect: true,
            columns: [
                { field: 'Numero_Parte_ID', title: '', width: 0, align: 'center', valign: 'bottom', sortable: true, visible: false },
                ...
                { field: 'Observaciones', title: 'Observations', width: 200, align: 'left', valign: 'bottom', sortable: true, clickToSelect: false },
                {
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Numero_Parte_ID + '" href="javascript:void(0)" data-numero=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Edit"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Numero_Parte_ID + '" href="javascript:void(0)" data-numero=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Remove"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_numero() {
    ...
    try {
        ...
        numero= new Object();
        numero.Nombre = $('#txt_nombre').val();
        numero.Observaciones = $('#txt_observaciones').val();
        numero.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(numero) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Numero_Partes_Controller.asmx/Alta',
            data: $data,
...
function _modificar_numero_parte() {
    var Numero_Parte = null;
    var isComplete = false;

    try {
        Numero_Parte = new Object();
        Numero_Parte.Numero_Parte_ID = parseInt($('#txt_numero_parte_id').val());
        Numero_Parte.Nombre = $('#txt_nombre').val();
        Numero_Parte.Observaciones = $('#txt_observaciones').val();
        Numero_Parte.Estatus_ID = parseInt($('#cmb_estatus').val());

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Numero_Parte) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Numero_Partes_Controller.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar_numero_parte(Numero_Parte_ID) {
    var numero_parte = null;

    try {
        numero_parte = new Object();
        numero_parte.Numero_Parte_ID = parseInt(Numero_Parte_ID);

        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(numero_parte) });

        $.ajax({
            type: 'POST',
            url: 'controllers/Numero_Partes_Controller.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags += '<div class="row">' +
            ...
            '        <input type="hidden" id="txt_numero_parte_id"/>' +
            '       </div> ' +
        ...
        $('#btn_guardar_datos').bind('click', function (e) {
            ...
            if ($('#txt_numero_parte_id').val() != null && $('#txt_numero_parte_id').val() != undefined && $('#txt_numero_parte_id').val() != '') {
                var _output = _validation('editar');
                if (_output.Estatus) {
                    if (_modificar_numero_parte()) {
                        _estado_inicial();
...
function btn_editar_click(numero) {
    var row = $(numero).data('numero');

    $('#txt_numero_parte_id').val(row.Numero_Parte_ID);
...
function btn_eliminar_click(numero) {
    var row = $(numero).data('numero');
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
               _eliminar_numero_parte(row.Numero_Parte_ID);
            }
            _estado_inicial();
        }
    });
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'controllers/Numero_Partes_Controller.asmx/Consultar_Numero_Parte_Por_Filtros',
                    data: $data,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_numero_partes').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validation(opcion) {
    ...
    if (!$('#txt_nombre').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#txt_nombre').val(), $('#txt_numero_parte_id').val(), 'nombre');
    ...
    if (!$('#cmb_estatus').parsley().isValid()) {
        ...
        var _Resultado = (opcion === 'alta') ?
            ...
            _validate_fields($('#cmb_estatus').val(), $('#txt_numero_parte_id').val(), 'estatus');
...
function _validate_fields(value, id, field) {
    ...
    try {
        Numero_Parte= new Object();
        if (id !== null)
            Numero_Parte.Numero_Parte_ID = parseInt(id);

        switch (field) {
            case 'nombre':
                Numero_Parte.Nombre = value;
                break;
            default:
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Numero_Partes_Controller.asmx/Consultar_Numero_Parte_Por_Nombre',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Clientes_Controller.asmx/Consultar_Estatus',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            cache: false,
