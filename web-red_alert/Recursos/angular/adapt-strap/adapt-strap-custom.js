                    ...
                    return {
                        pre: function preLink(scope, element, attrs) {
                            var loadingIconClass = attrs.loadingIconClass || $adConfig.iconClasses.loadingSpinner,
                                ngStyleTemplate = attrs.loadingIconSize ? 'ng-style="{\'font-size\': \'' + attrs.loadingIconSize + '\'}"' : '',
        ...
        function($parse, $http, $compile, $filter, $templateCache, $adConfig, adStrapUtils, adDebounce, adLoadLocalPage) {
            function controllerFunction($scope, $attrs) {
                ...
                $scope.expandAll = function() {
                    for (let index = 0; index < $scope.items.list.length; index++) {
                        adStrapUtils.addRemoveItemFromList(index, $scope.localConfig.expandedItems);
                    }
                };
                $scope.expandCell = function (event, index, item) {
					event.stopPropagation();
                    adStrapUtils.addRemoveItemFromList(index, $scope.localConfig.expandedItems);
                    if (adStrapUtils.itemExistsInList(index, $scope.localConfig.expandedItems)) {
                        var cellExpandCallback = $scope.$eval($attrs.cellExpandCallback);
                        if (cellExpandCallback) {
                            cellExpandCallback(item);
                        }
                    }
                };
                $scope.collapseCell = function (event, index) {
                    event.stopPropagation();
                    adStrapUtils.addRemoveItemFromList(index, $scope.localConfig.expandedItems);
                };
                $scope.expandCollapseRow = function(index) {
                ...
                $scope.$on('adTableLiteAction', function(event, data) {
                    // Exposed methods for external actions
                    var actions = { expandCollapseRow: $scope.expandCollapseRow, collapseAll: $scope.collapseAll, expandAll: $scope.expandAll, expandCell: $scope.expandCell, collapseCell: $scope.collapseCell };
                    if (data.tableName === $scope.attrs.tableName) {
                        data.action(actions);
                    }
                });
            ...
            return function(options) {
                ...
                if (angular.isDefined(options.localData)) {
                    var start = (options.pageNumber - 1) * options.pageSize,
                        ...
                        i, itemsObject = options.localData,
